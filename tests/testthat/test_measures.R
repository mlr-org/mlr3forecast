test_that("forecast measures", {
  ...
  for (key in keys) {
    ...
    if ((is.na(m$task_type) || m$task_type == "regr") && m$predict_type == "response") {
      perf = suppressWarnings(m$score(prediction = p, task = task, learner = learner, train_set = train_set))
...
test_that("MeasureMDA works", {
  ...
  expect_identical(suppressWarnings(pred$score(measure)), c(fcst.mda = 1.0))
  response = c(2, -2)
  pred = PredictionRegr$new(truth = truth, response = response, row_ids = seq_along(truth))
  expect_identical(suppressWarnings(pred$score(measure)), c(fcst.mda = 0.0))
  ...
  response = frev(truth)
  pred = PredictionRegr$new(truth = truth, response = response, row_ids = seq_along(truth))
  expect_identical(suppressWarnings(pred$score(measure)), c(fcst.mda = 0.5))
...
test_that("MeasureMDV works", {
  ...
  response = truth + 2
  pred = PredictionRegr$new(truth = truth, response = response, row_ids = seq_along(truth))
  expect_identical(suppressWarnings(pred$score(measure)), c(fcst.mdv = 1.0))
  ...
  truth = response = rep.int(7, 5)
  pred = PredictionRegr$new(truth = truth, response = response, row_ids = seq_along(truth))
  expect_identical(suppressWarnings(pred$score(measure)), c(fcst.mdv = 0.0))
})
...
test_that("MeasureMDPV works", {
  ...
  response = truth + 1L
  pred = PredictionRegr$new(truth = truth, response = response, row_ids = seq_along(truth))
  expect_identical(suppressWarnings(pred$score(measure)), c(fcst.mdpv = 1.0))
  ...
  truth = response = rep.int(7, 5)
  pred = PredictionRegr$new(truth = truth, response = response, row_ids = seq_along(truth))
  expect_identical(suppressWarnings(pred$score(measure)), c(fcst.mdpv = 0.0))
})
...
test_that("MeasureACF1 works", {
  ...
  pred = PredictionRegr$new(truth = truth, response = response, row_ids = seq_along(truth))
  expect_number(suppressWarnings(unname(pred$score(measure))), lower = -1, upper = 1)
  # single observation returns NA
  pred = PredictionRegr$new(truth = 1, response = 1, row_ids = 1L)
  expect_identical(suppressWarnings(unname(pred$score(measure))), NA_real_)
})
...
test_that("measures match fabletools reference implementation", {
  ...
  expect_equal(suppressWarnings(unname(pred$score(msr("fcst.mda")))), expected_mda)

  # MDA with custom reward/penalty
  expected_mda_custom = fabletools::MDA(resid, truth, reward = 1, penalty = -1)
  expect_equal(
    suppressWarnings(unname(pred$score(msr("fcst.mda", reward = 1, penalty = -1)))),
    expected_mda_custom
  )

  # MDV
  expected_mdv = fabletools::MDV(resid, truth)
  expect_equal(suppressWarnings(unname(pred$score(msr("fcst.mdv")))), expected_mdv)
  ...
  expect_equal(suppressWarnings(unname(pred$score(msr("fcst.acf1")))), expected_acf1)
