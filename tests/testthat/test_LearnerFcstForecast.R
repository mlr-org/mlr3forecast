...
test_that("symmetric quantile prediction works for h = 1 and h > 1", {
  ...
  pred1 = learner$predict_newdata(generate_newdata(train_task, n = 1L))
  expect_matrix(pred1$data$quantiles, nrows = 1L)

  pred6 = learner$predict_newdata(generate_newdata(train_task, n = 6L))
...
test_that("asymmetric quantile prediction works", {
  ...
  learner$train(task, 1:132)
  train_task = task$clone()$filter(1:132)
  pred = learner$predict_newdata(generate_newdata(train_task, n = 6L))
  ...
  qref = ref$predict_newdata(generate_newdata(train_task, n = 6L))$data$quantiles
...
test_that("quantile prediction with only the median works", {
  ...
  learner$train(task, 1:132)
  train_task = task$clone()$filter(1:132)
  pred = learner$predict_newdata(generate_newdata(train_task, n = 3L))
