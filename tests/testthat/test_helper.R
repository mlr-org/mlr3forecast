...
test_that("seq_order continues a month-end origin at month-end, not a fixed day", {
  # Apr-30 is April's last day, so steps snap to each target month's end
  expect_identical(
    seq_order(as.Date("2020-04-30"), "month", 3L),
    as.Date(c("2020-05-31", "2020-06-30", "2020-07-31"))
  )
  # Mar-30 is not month-end (March has 31 days), so the 30th is carried forward
  expect_identical(
    seq_order(as.Date("2020-03-30"), "month", 3L),
    as.Date(c("2020-04-30", "2020-05-30", "2020-06-30"))
  )
})
