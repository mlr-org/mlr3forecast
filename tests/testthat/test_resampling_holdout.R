test_that("fcst.holdout basic properties", {
  # only work with fcst task
  task = tsk("iris")
  resampling = rsmp("fcst.holdout", ratio = 0.8)
  expect_error(resampling$instantiate(task), "Resampling 'fcst.holdout' requires an ordered task.")
  ...
  resampling = rsmp("fcst.holdout", ratio = 0.5)$instantiate(task)
...
test_that("fcst.holdout works", {
  ...
  resampling = rsmp("fcst.holdout", ratio = 0.5)
  resampling$instantiate(task)
...
test_that("fcst.holdout repeated instantiation", {
  task = tsk("airpassengers")
  resampling = rsmp("fcst.holdout", ratio = 0.6)
