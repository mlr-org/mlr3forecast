...
PipeOpFcstAvg = R6Class(
  ...
  public = list(
    ...
    #'   Identifier of resulting object, default `"fcst.regravg"`.
    ...
    initialize = function(innum = 0L, collect_multiplicity = FALSE, id = "fcst.regravg", param_vals = list()) {
      super$initialize(innum, collect_multiplicity, id = id, param_vals = param_vals)
      # retype the channels from PredictionRegr to PredictionFcst so the averaged output stays a forecast
      # prediction and the surrounding graph's task_type is inferred as "fcst"
  ...
  private = list(
    weighted_avg_predictions = function(inputs, weights, row_ids, truth) {
      # reuse PipeOpRegrAvg's response/se aggregation, then re-wrap as a forecast prediction carrying the
      # time index and key columns shared by all inputs
...
#' @include zzz.R
register_po("fcst.regravg", PipeOpFcstAvg)
