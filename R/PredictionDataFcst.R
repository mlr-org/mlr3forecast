...
c.PredictionDataFcst = function(..., keep_duplicates = TRUE) {
  dots = list(...)
  assert_list(dots, "PredictionDataFcst")
  assert_flag(keep_duplicates)
  if (length(dots) == 1L) {
    return(dots[[1L]])
  }

  predict_types = names(mlr_reflections$learner_predict_types$fcst)
  ...
  elems = c(
    "row_ids",
    "truth",
    intersect(predict_types[[1L]], c("response", "se")),
  ...
  new_prediction_data(result, "fcst")
}
...
create_empty_prediction_data.TaskFcst = function(task, learner) {
  predict_types = mlr_reflections$learner_predict_types[["fcst"]][[learner$predict_type]]
  ...
  new_prediction_data(pdata, "fcst")
}
