...
(values pasted with `":"`, deduplicated with
[`make.unique()`](https://rdrr.io/r/base/make.unique.html) when key
values contain the separator). Set `key` to the task's key column name
to get predictions column-compatible with global forecasters such as
