...
The resulting [`PredictionFcst`](https://mlr3forecast.mlr-org.com/reference/PredictionFcst.html)
...
Several forecasters can be ensembled by branching with `gunion()` and averaging
their forecasts with `po("fcst.avg")`, which keeps the forecast prediction
type (so the time index, keys, and `autoplot()` survive the averaging). This
mirrors the idea behind the forecastHybrid package, but with any mix of classical
or ML learners.
